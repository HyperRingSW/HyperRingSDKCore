 ...
 *
 */
class AESWalletHRData(tag: Tag?) : HyperRingData(tag) {
    ...
    companion object {
         ...
         */
        fun createData(id: Long, name: String): AESWalletHRData {
            val demoNFCData = AESWalletHRData(id, "")
//            var jsonData = "{\"id\":$id,\"data\":\"{\\\"name\\\":\\\"$name\\\"}\"}"
            demoNFCData.encrypt("{\\\"wallet\\\":\\\"$name\\\"}")
            return demoNFCData
        }
    }
